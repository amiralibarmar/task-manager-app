...
MOD_LABEL = "Cmd" if platform.system() == "Darwin" else "Ctrl"
...
class TaskManagerApp:
    ...
    def build_menu(self):
        ...
        file_menu.add_command(label="New", accelerator=f"{MOD_LABEL}+N", command=self.new_file)
        file_menu.add_command(label="Open", accelerator=f"{MOD_LABEL}+O", command=self.load_tasks)
        file_menu.add_command(label="Save", accelerator=f"{MOD_LABEL}+S", command=self.save_tasks)
    ...
    def build_bindings(self):
        ...
        self.root.bind(f"<{MOD_KEY}-o>", lambda e: self.load_tasks())
        self.root.bind(f"<{MOD_KEY}-s>", lambda e: self.save_tasks())
